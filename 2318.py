...
# load data
train_data = pd.read_csv('2318data/2318.HK training.csv')
test_data = pd.read_csv('2318data/2318.HK test.csv')
# # 提取价格列
...
mape_test = round(mean_absolute_percentage_error(y_test, y_test_pred),4)
metrics_svr=np.array([r2_test,mse_test,rmse_test,mae_test,mape_test])
pd.DataFrame(metrics_svr,index = ['R2','MSE','RMSE','MAE','MAPE']).to_csv('Metrics_AdaBoost_以前.csv',header=False,index=True)
...
plt.ylabel('Values')
plt.legend()
plt.savefig(fname='折线图_AdaBoost_以前.png',dpi=600)
plt.show()
plt.clf()
...
mape_test = round(mean_absolute_percentage_error(y_test, y_test_pred),4)
metrics_svr=np.array([r2_test,mse_test,rmse_test,mae_test,mape_test])
pd.DataFrame(metrics_svr,index = ['R2','MSE','RMSE','MAE','MAPE']).to_csv('Metrics_AdaBoost_变化.csv',header=False,index=True)
...
plt.ylabel('Values')
plt.legend()
plt.savefig(fname='折线图_AdaBoost_变化.png',dpi=600)
plt.show()
plt.clf()